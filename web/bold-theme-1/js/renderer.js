...
export async function initializeRenderer({
  ...
  renderLoading(main, "Loading bold content...");
...
function renderSite({ manifest, siteConfig }) {
  ...
  renderNavigation(navContainer, siteConfig.navigation);
...
function renderHeader(container, site) {
  ...
  const headerEl =
    clone instanceof DocumentFragment ? clone.firstElementChild : clone;
  if (!(headerEl instanceof HTMLElement)) {
    return null;
  }

  const titleEl =
    headerEl.querySelector("[data-site-title]") || headerEl.querySelector("h1");
  const taglineEl = headerEl.querySelector("[data-site-tagline]");
  ...
  headerEl.id = container.id || "site-header";
  headerEl.setAttribute("aria-live", "polite");

  container.replaceWith(headerEl);
  return headerEl.querySelector("#site-nav");
}
...
function renderNavigation(container, navigation = []) {
  ...
  const list =
    container.querySelector(".site-nav__list") ||
    container.appendChild(document.createElement("ul"));

  if (!list.classList.contains("site-nav__list")) {
...
function renderTile(type, item, mediaBasePath) {
  switch (type) {
    ...
    case "article":
    default:
      return renderArticleTile(item, mediaBasePath);
  }
}

function renderArticleTile(item, mediaBasePath) {
  const template = useTemplate("tmpl-tile-article");
  const node = template.cloneNode(true);

  const media = node.querySelector("[data-tile-media]");
  const img = node.querySelector("[data-tile-image]");
  const imageLink = node.querySelector("[data-tile-image-link]");
  const caption = node.querySelector("[data-tile-image-caption]");
  ...
  const cta = node.querySelector("[data-tile-cta]");

  const slug = String(item.slug ?? "").replace(/^\/+/, "");
  const href = item.canonical_url ?? (slug ? `/posts/${slug}/` : "#");

  const heroVariant = selectVariant(item.hero_media, ["large", "thumb", "original"]);
  if (media && img && heroVariant) {
...
function renderGalleryTile(item, mediaBasePath) {
  ...
  if (count) {
    const total = item.asset_count ?? 0;
...
function renderAudioTile(item, mediaBasePath) {
  ...
  const coverVariant = selectVariant(item.hero_media, ["large", "thumb", "original"]);
  if (media && coverVariant && img) {
    img.src = resolveMediaPath(coverVariant.path, mediaBasePath);
    img.alt = item.hero_media?.alt_text ?? titleText;
    media.hidden = false;
  } else if (media) {
    media.hidden = true;
  }
